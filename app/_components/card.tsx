...
export const FeatureCard = ({  id, gradient, children, img }: FeatureCardProps) => {
  ...
  return (
    ...
    >
      ...
      <button onClick={() => setFullScreenFeature(id)} className='absolute bottom-2 right-2 bg-black mt-2 px-4 py-1 shadow-lg rounded-xl text-white'>Show</button>
      </div>
      {/* <Image className={classNames(inVIewFeature === id ? 'opacity-50' : 'opacity-0','aspect-square w-full h-[90%] rounded-xl shadow-lg')} width={500} height={500} src={img} alt={id}/> */}
